...
3. 如果每条记录能确认款号、颜色、尺码但没有写数量，数量默认为 1。
4. 不需要记录品名、商品描述、建议体重、单价、金额等信息。
5. 尺码统一换算为字母码：105 = 165 = M；110 = 170 = L；115 = 175 = XL；120 = 180 = 2XL；125 = 185 = 3XL；130 = 190 = 4XL；135 = 195 = 5XL。
6. 如果图片里是横向尺码表，请按颜色逐行读取各尺码数量，再转成竖向明细。
7. 如果同一款号、同一颜色、同一尺码出现多次，请合并数量。
8. 图片中的“共 xx”“合计 xx”只作为校验总数，不作为单独明细行。
9. 颜色名称按用户习惯统一：雾霾蓝 = 雾蓝。
10. 如果图片里款号是 7262，输出款号写成 T262。
11. 看不清的内容不要硬猜，在 notes 里标注不确定项。
12. 合计数只用于核对。如果表格明细相加与图片合计不一致，需要在 notes 里说明哪里可能看错。
`.trim();
